...
public func createReacords(){
    ...
    for index in 0...9 {
        ...
        if currentPlayer.nameLabel.text == nil{
            currentPlayer.nameLabel.text = "---"
        }
        if currentPlayer.scoreLabel.text == nil{
            currentPlayer.scoreLabel.text = "---"
        }
...
public func updateRecords(){
    ...
    ///Находим номер человека, у которого меньше очков
    for index in 0...9{
        let stringScore = playersArray[index].scoreLabel.text ?? ""
        let intScore = Int(stringScore) ?? 999999
        if score > intScore{
            sortIndex = index
    ...
    ///Если такой человек есть,  вместо него подставляем нового победителя, всех остальных ставим ниже. Иначе cтавим игрока в самый конец
    if isSortIndexExists == true{
        ...
        for index in 1...9{
            if playersArray[index].nameLabel.text == "---"{
                playersArray[index].nameLabel.text = currentNameLabel.text
                playersArray[index].scoreLabel.text = currentScoreLabel.text
                UserDefaults.standard.set(playersArray[index].nameLabel.text, forKey: "\(index)Name")
                UserDefaults.standard.set(playersArray[index].scoreLabel.text, forKey: "\(index)Score")
